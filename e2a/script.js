...
function createQuestionElement(item, index) {
    ...
    questionDiv.className = 'question'; // Убираем автоматическое добавление 'active'
...
function checkAnswer() {
    ...
    if (quizData[currentQuestion].type === "radio") {
        ...
        answers[currentQuestion] = selected;
    } else if (quizData[currentQuestion].type === "date") { // Добавляем поддержку date
...
// Генерация кода с учетом баллов
...
function showResults() {
    ...
    quizData.forEach((question, index) => {
        ...
        } else if (question.type === "date") { // Добавляем проверку для date
